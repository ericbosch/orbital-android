...
- [x] **1.1** Completar `NsdDiscoveryService`: emitir `Flow<List<Server>>` con servidores descubiertos vía mDNS
- [x] **1.2** Añadir entrada manual de IP/puerto en `ScanScreen` como fallback
- [x] **1.3** Implementar `OrbitalApiClient.connect(server, token)`: validar URL + ping `GET /api/health`
- [x] **1.4** Persistir servidor + token en `DataStore` (dentro de `OrbitalRepository`)
- [x] **1.5** Conectar flujo onboarding completo: Scan → DetectAgents (NSD) → Ready → Home
- [x] **1.6** `MainViewModel`: estado `Connected / Disconnected / Scanning`

## Fase 2 — Proyectos y sesiones
> Objetivo: listar y navegar proyectos/sesiones reales del servidor.

- [x] **2.1** `GET /api/projects` → `OrbitalRepository.getProjects(): Flow<List<Project>>`
- [x] **2.2** `GET /api/sessions?projectName=X` → `getSessionsForProject()`
- [x] **2.3** `SessionsScreen`: reemplazar datos mock por datos reales
- [x] **2.4** `HomeScreen`: sesiones recientes reales (últimas 5 por `updatedAt`)
- [x] **2.5** Modelo `Project` en `domain/` con campos básicos (name, path, sessionCount)

## Fase 3 — Chat y streaming
> Objetivo: conversación funcional con streaming de respuestas.

- [x] **3.1** `GET /api/sessions/:id/messages` → cargar historial al abrir chat
- [x] **3.2** WebSocket connect: `ws://<host>/ws?token=<token>&sessionId=<id>`
- [x] **3.3** Enviar mensaje: evento `{ type: "message", content: "...", sessionId: "..." }`
- [x] **3.4** Recibir streaming: parsear eventos `output` (texto incremental), `tool_use`, `done`, `error`
- [x] **3.5** `ChatScreen`: burbujas de mensaje, indicador de pensamiento (spinner), tool calls colapsables
...
- [x] **5.1** `AppearanceStore`: guardar tema + accent en `DataStore` (ya existe, completar)
- [x] **5.2** `SettingsScreen`: conectar selector de tema → aplicar en tiempo real
- [ ] **5.3** Gestión de servidores: añadir/eliminar/editar en Settings
- [x] **5.4** Token de autenticación: input en Settings + validación contra servidor
- [x] **5.5** `TroubleshootScreen`: ping al servidor, ver estado daemon, logs recientes
